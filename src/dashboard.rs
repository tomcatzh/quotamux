...
<body><main>
  <header><div><h1>QuotaMux</h1><div id="subtitle" class="muted">Layered multi-provider gateway</div></div><div id="updated" class="muted">loading…</div></header>
  ...
  <h2>Recent requests</h2><div class="scroll"><table><thead><tr><th>Time</th><th>Protocol</th><th>Served model</th><th>Provider</th><th>Upstream model</th><th>Status</th><th>Fallback</th><th>Latency</th><th>Tokens</th></tr></thead><tbody id="requests"></tbody></table></div>
...
async function refresh(){
 ...
 document.querySelector('#subtitle').textContent=(status.models||[]).length?`Served models: ${status.models.join(', ')}`:'No served models';
 const cards=[['Requests',n(stats.requests.total)],['Logical errors',pct(stats.requests.errors,stats.requests.total)],['Fallbacks',n(stats.requests.fallbacks)],['Output tokens',n(stats.requests.output_tokens)],['Traffic',`${(stats.requests.bytes/1048576).toFixed(2)} MiB`]];
 ...
 document.querySelector('#providers').innerHTML=Object.entries(stats.providers||{}).map(([provider,r])=>`<tr><td>${esc(provider)}</td><td>${n(r.attempts)}</td><td>${n(r.successes)}</td><td>${n(r.errors)}</td><td>${pct(r.cache_hit_tokens,r.cache_hit_tokens+r.cache_miss_tokens)} <span class="muted">(${pct(r.cache_reported_attempts,r.attempts)} coverage)</span></td><td>$${Number(r.cost_usd||0).toFixed(6)}</td></tr>`).join('');
 document.querySelector('#requests').innerHTML=recent.requests.map(r=>`<tr><td>${new Date(r.started_at_ms).toLocaleTimeString()}</td><td>${esc(r.protocol)}</td><td>${esc(r.served_model||'—')}</td><td>${esc(r.provider||'—')}</td><td>${esc(r.upstream_model||'—')}</td><td>${r.status}</td><td>${r.fallback?'yes':'no'}</td><td>${r.total_ms} ms</td><td>${n(r.usage.total_tokens)}</td></tr>`).join('');
...
mod tests {
    ...
    #[test]
    fn dashboard_is_model_aware_and_has_no_single_provider_cards() {
        assert_eq!(
            INDEX_HTML
                .matches("<th>Served model</th><th>Provider</th><th>Upstream model</th>")
                .count(),
            1
        );
        assert!(INDEX_HTML.contains("<th>Provider</th><th>Attempts</th>"));
        assert!(INDEX_HTML.contains("Object.entries(stats.providers||{})"));
        assert!(INDEX_HTML.contains("r.served_model||'—'"));
        assert!(INDEX_HTML.contains("r.upstream_model||'—'"));
        assert!(!INDEX_HTML.contains("r.served_model||r.requested_model"));
        assert!(!INDEX_HTML.contains("r.upstream_model||r.requested_model"));
        assert!(!INDEX_HTML.contains("Active provider"));
        assert!(!INDEX_HTML.contains("Go circuit"));
        assert!(!INDEX_HTML.contains("DeepSeek balance"));
        assert!(!INDEX_HTML.contains("DeepSeek V4 Flash 0731 gateway"));
    }
}
