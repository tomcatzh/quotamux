...
protocol-translation, configurable inference-body-limit, and memory-only
prompt-prefix-affinity test runs. API keys, prompt bodies, and model response
bodies are excluded.
...
| --- | ---: | --- |
| Library/unit tests | 126 passed, 0 failed | configuration and protocol matrices, including the 32 MiB default inference-body limit and positive-limit validation; the current OpenCode Go endpoint catalog and Anthropic/Responses vision-block conversion; backend/adapter naming and persisted-record compatibility; source-free configuration parse errors; provider-specific OpenCode Go, Kimi Code, Kimi Open Platform, and Zhipu Coding Plan errors; bounded retry hints and legacy-deadline clamping; generation-safe concurrent circuits; circuit-neutral ambiguous provider rejections; half-open follower notification; abandoned and expired half-open leases; collision-free circuit-state migration; bounded and redacted upstream errors; HTTP-200 error envelopes; restart recovery; storage; streaming; affinity; timeout defaults and validation; wait-budget-derived `Retry-After` rounding; and compatibility |
| Local gateway end-to-end | 55 passed, 0 failed | the existing protocol/routing matrix plus an exact-boundary request larger than 2 MiB containing long text and two Base64 image data URIs, verified both when accepted upstream and when rejected with a persisted pre-handler `413` and zero upstream attempts; bounded follower waiting for in-flight half-open probes in JSON and streaming request paths, continued waiting when the first of multiple probes fails, concrete-target deduplication across route layers, recovery without immediate `503`, timeout with `Retry-After`, Kimi Code unknown-`403` retry recovery for JSON and streaming requests, retry-exhausted per-request fallback without circuit poisoning, eight concurrent unknown-`403` recoveries leaving the shared circuit closed, known Kimi quota `403` bypassing retry, explicit `fallback_exhausted` recording when every circuit is skipped, a dropped half-open streaming probe, eight simultaneous delayed `429` responses counting as one circuit failure, the official Go quota fixture with an extreme `Retry-After` capped to 15 minutes, HTTP-200 error-envelope fallback, provider-aware SSE quota classification before and after response commitment, generic malformed or semantic stream failures, a stream pause exceeding the ordinary read timeout while remaining below the streaming timeout, downstream heartbeat comments during that pause, and a precisely classified streaming read timeout |
| Configurable body-limit release on isolated `8081` and production `8080` | passed | Candidate and production used the exact image `sha256:83a7b265e70384ee4376e2af940815b0538fb62ffe8c3110a23b046cbf871d20`. A synthetic 2.2 MB `/v1/messages/count_tokens` request returned HTTP 200 with 550,011 estimated input tokens on both instances, and the ordered attempt-ID history remained unchanged. The complete Chat, streaming Chat, tools, Responses, streaming Responses, Anthropic, streaming Anthropic, count-tokens, and persisted-statistics smoke suite passed first on the isolated candidate and then on production through Zhipu Coding Plan. An initial candidate Kimi request was rejected upstream with its known quota `403` and was correctly classified as `provider_quota`. Production retained its existing data volume, `restart: unless-stopped`, healthy status, and zero restarts. |
| Probe-wait release candidate on isolated `8081` | passed, 5 complete smoke suites | The exact candidate image used a separate temporary data volume and passed health, dashboard loading, Chat, streaming Chat, tools, Responses, streaming Responses, Anthropic, streaming Anthropic, count-tokens, and persisted-statistics checks for all configured models. The three DeepSeek models completed through DeepSeek Official, GLM completed through Zhipu Coding Plan after one transient upstream failure and a complete successful rerun, and Kimi K3 completed through Kimi Code. The container remained healthy with zero restarts. |
...
| Unknown-`403` retry candidate on isolated `8081` | passed, 2 complete smoke suites | The release candidate used a separate temporary configuration and data directory; Kimi K3 passed Chat, streaming Chat, tools, Responses, streaming Responses, Anthropic, streaming Anthropic, count-tokens, and persisted-statistics checks with `kimi-code` as the selected backend; DeepSeek V4 Flash passed the same matrix with `opencode-go`; the candidate stopped cleanly and production `8080` retained its original process and start time |
...
| Static and frontend checks | passed | `cargo fmt --all -- --check`, `cargo clippy --locked --all-targets --all-features -- -D warnings`, `cargo test --locked --all-targets --all-features -- --test-threads=1`, `docker build`, and `git diff --check`; the existing clean frontend dependency installation and Vite production-build evidence remains valid |
